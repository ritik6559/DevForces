...
const ChallengeIDE = () => {
  ...
  const [rightTab, setRightTab] = useState<"task" | "results" | "submissions">(
    "task",
  );
    ...
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      ...
      <div className="h-10 flex items-center justify-between px-3 border-b border-border bg-surface shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-foreground font-medium truncate max-w-50">
      ...
      <div className="flex flex-1 overflow-hidden">
                ...
                {files.map((f) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!openTabs.includes(f.path))
                        setOpenTabs([...openTabs, f.path]);
                    }}
        ...
        {!leftOpen && (
          <button
            ...
            className="border-r border-border bg-surface px-1 flex items-center text-muted-foreground hover:text-foreground transition-colors"
          >
          ...
          <div className="h-9 flex items-center bg-surface border-b border-border overflow-x-auto shrink-0">
            {openTabs.map((path) => (
              ...
              >
                ...
                {files.find((f) => f.path === path)?.locked && (
                  ...
                  {challenge.description.split("\n").map((line, i) => {
                    if (line.startsWith("# "))
                      return (
                        <h3
                          key={i}
                          className="text-foreground font-heading font-bold text-base mt-4 mb-2"
                        >
                          {line.slice(2)}
                        </h3>
                      );
                    if (line.startsWith("## "))
                      return (
                        <h4
                          key={i}
                          className="text-foreground font-heading font-semibold text-sm mt-3 mb-1"
                        >
                          {line.slice(3)}
                        </h4>
                      );
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {challenge.allowedPackages.map((pkg) => (
                      <span
                        key={pkg}
                        className="font-mono text-xs bg-muted text-foreground px-2 py-1 rounded"
                      >
                        {pkg}
                      </span>
                    ))}
                    ...
                    >
                      ...
                      <div className="space-y-2 text-xs text-muted-foreground">
                        <p className="bg-muted rounded p-2">
                          ✅ Great use of bcrypt for password hashing with
                          proper salt rounds.
                        </p>
                        <p className="bg-muted rounded p-2">
                          ⚠️ Missing input validation — add a check for missing
                          fields before processing.
                        </p>
                        <p className="bg-muted rounded p-2">
                          💡 Set the Content-Type header explicitly with{" "}
                          <code className="text-foreground">res.json()</code>{" "}
                          instead of{" "}
                          <code className="text-foreground">res.send()</code>.
                        </p>
                      </div>
                    </motion.div>
                  </motion.div>
                  ...
                  <div className="space-y-1">
                    {submissions.map((s) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-xs font-mono uppercase ${s.status === "completed" ? "text-success" : "text-muted-foreground"}`}
                        >
                          {s.status}
                        </span>
                      </div>
          ...
          {rightTab === "task" && (
            <div className="border-t border-border p-3 space-y-2 shrink-0">
              ...
              <p className="text-[10px] text-muted-foreground text-center">
                Max 1 submission per 30 seconds
              </p>
            </div>
          )}
        </div>
